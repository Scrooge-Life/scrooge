defmodule ScroogeWeb.CoreComponents do
  @moduledoc """
  ...
  import ScroogeWeb.Gettext, warn: false
  ...
  def simple_form(assigns) do
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
      <.form :let={f} for={@for} as={@as} {@rest}>
        <div class="bg-white py-8 px-4 shadow space-y-4 sm:rounded-lg sm:px-10">
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 rounded-lg bg-mint-300 hover:bg-mint-400 py-2 px-3",
  ...
  def input(%{type: "checkbox"} = assigns) do
    ~H"""
    <label phx-feedback-for={@name} class="flex items-center gap-2 text-sm leading-6 text-zinc-800">
      <input
        type="checkbox"
        id={@id || @name}
        name={@name}
    ...
    <div phx-feedback-for={@name}>
      ...
      <textarea
        ...
        class={[
          ...
          "text-zinc-900 focus:border-sky-400 focus:outline-none focus:ring-4 focus:ring-sky-800/5 sm:text-sm sm:leading-6",
          "phx-no-feedback:border-zinc-300 phx-no-feedback:focus:border-zinc-400 phx-no-feedback:focus:ring-sky-800/5"
        ]}
        {@rest}
      ><%= @value %></textarea>
      <.error :for={msg <- @errors} message={msg} />
    ...
    <div phx-feedback-for={@name}>
      ...
      <input
        ...
        class={[
          ...
          "phx-no-feedback:border-zinc-300 phx-no-feedback:focus:border-sky-400 phx-no-feedback:focus:ring-sky-800/5"
        ]}
        {@rest}
      />
      <.error :for={msg <- @errors} message={msg} />
  ...
  defp input_border([] = _errors),
    do: "border-zinc-300 focus:border-sky-400 focus:ring-sky-800/5"
  ...
  def error(assigns) do
    ~H"""
    <p class="phx-no-feedback:hidden mt-3 flex gap-2 text-sm leading-6 text-rose-600">
  ...
  def hide_dropdown(js \\ %JS{}, selector) do
    JS.hide(js,
      to: selector,
      time: 75,
      transition:
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ScroogeWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ScroogeWeb.Gettext, "errors", msg, opts)
    end
  end
