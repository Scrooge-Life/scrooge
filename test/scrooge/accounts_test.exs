defmodule Scrooge.AccountsTest do
  ...
  alias Scrooge.Accounts.{User, UserToken}
  ...
  describe "budgets" do
    ...
    test "get_budget!/1 returns the budget with given id" do
      budget = budget_fixture()
      assert Accounts.get_budget!(budget.id) == budget
    end

    test "create_budget/1 with valid data creates a budget" do
      valid_attrs = %{name: "some name"}

      assert {:ok, %Budget{} = budget} = Accounts.create_budget(valid_attrs)
      assert budget.name == "some name"
    end

    test "create_budget/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Accounts.create_budget(@invalid_attrs)
    end
    ...
    test "update_budget/2 with valid data updates the budget" do
      ...
      assert {:ok, %Budget{} = budget} = Accounts.update_budget(budget, update_attrs)
      assert budget.name == "some updated name"
    end

    test "update_budget/2 with invalid data returns error changeset" do
      budget = budget_fixture()
      assert {:error, %Ecto.Changeset{}} = Accounts.update_budget(budget, @invalid_attrs)
      assert budget == Accounts.get_budget!(budget.id)
    end

    test "delete_budget/1 deletes the budget" do
      budget = budget_fixture()
      assert {:ok, %Budget{}} = Accounts.delete_budget(budget)
      assert_raise Ecto.NoResultsError, fn -> Accounts.get_budget!(budget.id) end
    end

    test "change_budget/1 returns a budget changeset" do
      budget = budget_fixture()
      assert %Ecto.Changeset{} = Accounts.change_budget(budget)
    end
  end
end
