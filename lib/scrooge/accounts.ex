defmodule Scrooge.Accounts do
  ...
  alias Scrooge.Accounts.{Budget, User, UserToken, UserNotifier}
  alias Scrooge.Repo
  ...
  ## Examples
      ...
      iex> deliver_user_confirmation_instructions(user, &url(~p"/settings/confirm-account/#{&1}"))
      {:ok, %{to: ..., body: ...}}

      iex> deliver_user_confirmation_instructions(confirmed_user, &url(~p"/settings/confirm-account/#{&1}"))
  ...
  @doc """
  Returns the list of budgets for a given user.
  ...
  @doc """
  Creates a budget for the given User.

  ## Examples

      iex> create_budget(%User{}, %{field: value})
      {:ok, %Budget{}}

      iex> create_budget(%User{}, %{field: bad_value})
      {:error, %Ecto.Changeset{}}

  """
  def create_budget(user, attrs \\ %{}) do
    %Budget{}
    |> Budget.create_changeset(attrs, user)
    |> Repo.insert()
  end
